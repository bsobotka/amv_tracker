...
from fetch_video_info import fvi_TEMP as fetch_vid_info
...
class Worker(QtCore.QObject):
	...
	def run(self):
		...
		if self.url_type == 'org':
			starting_num = int(self.url.split('id=')[1])
			video_urls = ['https://www.animemusicvideos.org/members/members_videoinfo.php?vid_id=' + str(i) for i in
						  range(starting_num, starting_num + 9698)]
		...
		for lnk in video_urls:
			...
			if vid_data != {}:
				...
				for k, v in vid_data.items():
					if k == 'release_date':
						if vid_data['release_date'][1] == 0 or vid_data['release_date'][2] == 0:
							vid_entry_dict['release_date'] = ''
							vid_entry_dict['release_date_unknown'] = 1
						else:
							if vid_data['release_date'][1] <= 9:
								mo = '0' + str(vid_data['release_date'][1])
							else:
								mo = str(vid_data['release_date'][1])

							if vid_data['release_date'][2] <= 9:
								day = '0' + str(vid_data['release_date'][2])
							else:
								day = str(vid_data['release_date'][2])
							vid_entry_dict['release_date'] = vid_data['release_date'][0] + '/' + mo + '/' + day
							vid_entry_dict['release_date_unknown'] = 0

					elif k == 'video_footage':
						ftg_fixed = '; '.join(vid_data['video_footage'])
						vid_entry_dict['video_footage'] = ftg_fixed

					elif k == 'video_length':
						if vid_data['video_length'][0] == -1 or vid_data['video_length'] == -1:
							vid_entry_dict['video_length'] = ''
						else:
							vid_entry_dict['video_length'] = (60 * vid_data['video_length'][0]) + vid_data['video_length'][1]
					...
					if k == 'release_date':
						if self.url_type == 'youtube' or self.url_type == 'playlist':
							...
							if dct['release_date'][1] == 0 or dct['release_date'][2] == 0:
								...
								existing_entry['release_date_unknown'] = 0
...
class FetchWindow(QtWidgets.QMainWindow):
	def __init__(self, window_type='profile'):
		...
		elif self.window_type == 'test':
			msg = 'Enter starting URL.'
			url_label = 'test'
