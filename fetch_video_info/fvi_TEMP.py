...
def download_data(url, site, url_type='video'):
	...
	if site == 'org':
		...
		if 'members_videoinfo' in url:
			test_text = soup.find('span', {'class': 'infoTitle'}).get_text().strip()
			if test_text == 'Does not exist:' or test_text == 'Deleted:':
				out_dict = dict()
				print('hey')
			else:
				...
				try:
					genre_tags_raw = soup.find('ul', {'class': 'videoCategory'}).find_all('li')
					genre_tags = '; '.join([elem.get_text().strip().lower() for elem in genre_tags_raw])
				except:
					genre_tags = ''
