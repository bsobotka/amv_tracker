	...
	def import_btn_clicked(self):
		...
		if self.importDrop.currentText() == 'Previous AMV Tracker version':
			...
			for sht_ind in range(0, book.nsheets):
				...
				self.pBar.show()
				self.pBar.setRange(0, sheet.nrows)
				...
				for row in range(1, sheet.nrows):
					...
					"""cursor.execute(
						'UPDATE sub_db_{} SET (primary_editor_username, video_title, my_rating, star_rating, '
						...
						'?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) WHERE video_id = ?'.format(
							...
							'my_rating'], field_dict['star_rating'], field_dict['tags_1'],
						               field_dict['release_date'],
						               ...
						               field_dict['sequence'], field_dict['date_entered'], field_dict['notable'],
						               field_dict[
								           'favorite'], field_dict['video_id']))"""

					pBar_int += 1
					print(pBar_int)
					self.pBar.setValue(pBar_int)
				self.pBar.close()
		...
		self.compl_window = QtWidgets.QMessageBox(QtWidgets.QMessageBox.Information, 'Complete',
		                                          'This database has been imported into AMV Tracker.')
		self.compl_window.exec_()
		"""else:
			f_path = QtWidgets.QFileDialog.getOpenFileName(self, 'Select CSV file', '', 'CSV file (*csv)')[0]
			db_mgmt_import_db.Import_DB(f_path, 'csv')"""
