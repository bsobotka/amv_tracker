	...
	def run(self):
		...
		for sht_ind in range(0, book.nsheets):
			...
			cursor.execute(
				...
				'"date_entered"	TEXT, "play_count"	INTEGER, "vid_thumb_path" TEXT, "sub_db" TEXT, "video_source" TEXT, '
...
class DataMgmtSettings(QtWidgets.QWidget):
	def __init__(self):
		...
		self.exportToCSVButton = QtWidgets.QPushButton('Export to CSV')
		self.exportToCSVButton.setFixedWidth(150)
		self.gridLayout.addWidget(self.exportToCSVButton, grid_v_index, 0, alignment=QtCore.Qt.AlignCenter)
		grid_v_index += 1
		...
		## Signals/slots
		# DB operations
		self.importButton.clicked.connect(self.reset_tag_settings)
		self.importButton.clicked.connect(self.import_btn_clicked)
		self.exportToCSVButton.clicked.connect(self.export_btn_clicked)
		self.newDBButton.clicked.connect(self.create_db)
		self.changeCurrDBButton.clicked.connect(self.select_db)
		...
		# Sub-DB operations
		self.addSubDBButton.clicked.connect(self.add_subdb)
		self.renameSubDBsButton.clicked.connect(self.rename_subdb)
		self.deleteSubDBButton.clicked.connect(self.delete_subdb)
		self.clearAllDataButton.clicked.connect(lambda: self.clear_data(del_all=True))
		self.clearSelectDataButton.clicked.connect(self.clear_data)
	...
	def export_btn_clicked(self):
		...
		msg_str = 'Please select the sub-DB you would like\nto export:'
		subdb_list = [k for k, v in common_vars.sub_db_lookup().items()]
		subdb_list.sort(key=lambda x: x.casefold())
		subdb_list.remove('Main database')
		subdb_list.insert(0, 'Main database')
		win_tit = 'Choose sub-DB'

		choose_subdb_win = generic_dropdown.DropdownWindow(msg_str, subdb_list, win_tit)
		if choose_subdb_win.exec_():
